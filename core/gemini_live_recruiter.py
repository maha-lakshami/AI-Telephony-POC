#!/usr/bin/env python3
"""
Gemini Realtime Sales Bot - Enhanced Production Version with Multi-Sample Rate Support
Bridges Exotel WebSocket with Gemini Realtime API for natural conversations
...
class GeminiRealtimeSalesBot:
    def __init__(self):
        ...
        # Gemini Configuration - SECURE: Load from environment variables
        self.gemini_api_key = Config.GEMINI_API_KEY
        self.gemini_model = Config.OPENAI_MODEL
        self.gemini_voice = Config.OPENAI_VOICE
        ...
        logger.info("🤖 Enhanced Gemini Realtime Sales Bot initialized!")
    ...
    async def handle_exotel_connected(self, stream_id: str, data: dict):
        ...
        # Start enhanced Gemini Realtime connection
        await self.connect_to_Gemini_enhanced(stream_id)
    ...
    async def handle_exotel_media(self, stream_id: str, data: dict):
        ...
        # **ENHANCED: Auto-establish Gemini connection if missing**
        if stream_id not in self.Gemini_connections:
            logger.warning(f"⚠️ No Gemini connection for {stream_id} - ESTABLISHING NOW")
            await self.connect_to_Gemini_enhanced(stream_id)
            ...
            if stream_id not in self.Gemini_connections:
                logger.error(f"❌ Failed to establish Gemini connection for {stream_id}")
                return
        
        if stream_id in self.Gemini_connections:
            ...
            logger.warning(f"⚠️ Still no Gemini connection for {stream_id} after connection attempt")
    ...
    async def _process_variable_chunks(self, stream_id: str, sample_rate: int):
        ...
        while len(buffer) >= min_chunk_bytes:
            ...
            # Send to Gemini with enhanced format selection
            await self._send_audio_to_Gemini(stream_id, chunk, sample_rate)
    ...
    async def _send_audio_to_Gemini(self, stream_id: str, chunk: bytes, sample_rate: int):
        """Send audio chunk to Gemini with enhanced format handling"""
        try:
            # Get Gemini connection config
            Gemini_config = self.Gemini_connections[stream_id]
            input_format = Gemini_config.get("input_format", "raw/slin")
            
            # Convert audio based on sample rate and format
            if input_format == "pcm16" and sample_rate >= 16000:
                # High quality PCM for 16kHz+ 
                Gemini_audio = chunk  # Already PCM16
            else:
                # Convert to G.711 u-law for lower sample rates or telephony compatibility
                Gemini_audio = self.convert_pcm_to_ulaw(chunk)
            
            Gemini_audio_b64 = base64.b64encode(Gemini_audio).decode()
            
            # Send to Gemini Realtime API
            Gemini_msg = {
                "type": "input_audio_buffer.append",
                "audio": Gemini_audio_b64
            }
            
            Gemini_ws = Gemini_config["websocket"]
            await Gemini_ws.send(json.dumps(Gemini_msg))
            
            logger.debug(f"📤 AUDIO SENT TO Gemini: {len(chunk)} bytes PCM → {len(Gemini_audio)} bytes {input_format}")
    ...
    async def handle_exotel_clear(self, stream_id: str, data: dict):
        ...
        if stream_id in self.Gemini_connections:
            try:
                Gemini_ws = self.Gemini_connections[stream_id]["websocket"]
                ...
                if self.exotel_enhanced_events:
                    ...
                    await Gemini_ws.send(json.dumps(cancel_response_msg))
                    logger.info(f"🛑 CANCELLED ONGOING RESPONSE (enhanced) for {stream_id}")
                    
                    # 2. Clear Gemini's input audio buffer
                    clear_input_msg = {
                        "type": "input_audio_buffer.clear"
                    }
                    await Gemini_ws.send(json.dumps(clear_input_msg))
                    logger.info(f"🧹 CLEARED Gemini INPUT BUFFER (enhanced) for {stream_id}")
                else:
            ...
            logger.warning(f"⚠️ No Gemini connection to clear for {stream_id}")

    async def _commit_audio_buffer(self, stream_id: str):
        """Commit any remaining audio in buffer to Gemini (enhanced feature)"""
        if stream_id not in self.audio_buffers:
            return
        ...
        if len(buffer) > 0:
            ...
            if len(buffer) >= min_chunk_bytes:
                await self._send_audio_to_Gemini(stream_id, buffer, sample_rate)
                self.audio_buffers[stream_id] = b""
    ...
    async def connect_to_Gemini_enhanced(self, stream_id: str):
        """Establish enhanced connection to Gemini Realtime API with dynamic configuration"""
        try:
            sample_rate = self.connection_sample_rates.get(stream_id, self.default_sample_rate)
            logger.info(f"🔗 CONNECTING TO Gemini (ENHANCED) for {stream_id} @ {sample_rate}Hz")
            
            # Enhanced URL for latest Gemini Realtime API
            url = f"wss://api.Gemini.com/v1/realtime?model={self.gemini_model}"
            ...
            headers = [
                ...
                ("Gemini-Beta", "realtime=v1")
            ]
            
            # Connect to Gemini Realtime API with enhanced SSL context
            ...
            session_config = Config.get_enhanced_session_config(sample_rate, self.gemini_voice)
            
            self.Gemini_connections[stream_id] = {
                "websocket": Gemini_ws,
                "start_time": time.time(),
            ...
            if stream_id in self.exotel_connections:
                self.exotel_connections[stream_id]["Gemini_connected"] = True
            
            logger.info(f"✅ ENHANCED Gemini CONNECTED for {stream_id} @ {sample_rate}Hz")
            logger.info(f"🎵 Audio Format: {session_config['input_audio_format']} → {session_config['output_audio_format']}")
            
            # Configure enhanced Gemini session
            await self.configure_Gemini_session_enhanced(stream_id)
            
            # Start listening to Gemini responses
            asyncio.create_task(self.handle_Gemini_responses_enhanced(stream_id, Gemini_ws))
            
        except Exception as e:
            logger.error(f"❌ Failed to connect to Gemini (enhanced): {e}")
            logger.error(f"Error type: {type(e).__name__}")
            if "SSL" in str(e):
                logger.error("💡 SSL Error - trying with insecure SSL context")
            elif "authentication" in str(e).lower():
                logger.error("💡 Authentication Error - check Gemini API key")
            elif "websocket" in str(e).lower():
                logger.error("💡 WebSocket Error - check connection and headers")

    async def configure_Gemini_session_enhanced(self, stream_id: str):
        """Configure enhanced Gemini Realtime session"""
        try:
            Gemini_connection = self.Gemini_connections[stream_id]
            Gemini_ws = Gemini_connection["websocket"]
            session_config = Gemini_connection["session_config"]
            sample_rate = Gemini_connection["sample_rate"]
            ...
            logger.info(f"🔧 ENHANCED Gemini SESSION CONFIGURED for {stream_id}")
            logger.info(f"   🎵 Sample Rate: {sample_rate}Hz")
            ...
            logger.error(f"❌ Error configuring enhanced Gemini session: {e}")

    async def send_initial_greeting_enhanced(self, stream_id: str):
        """Send enhanced initial sales greeting through Gemini"""
        try:
            Gemini_ws = self.Gemini_connections[stream_id]["websocket"]
            sample_rate = self.connection_sample_rates.get(stream_id, self.default_sample_rate)
    ...
    async def handle_Gemini_responses_enhanced(self, stream_id: str, Gemini_ws):
        """Handle enhanced responses from Gemini Realtime API"""
        try:
            async for message in Gemini_ws:
                try:
                    ...
                    logger.debug(f"🤖 ENHANCED Gemini EVENT: {event_type} for {stream_id}")
                    
                    if event_type == "response.audio.delta":
                        await self.handle_Gemini_audio_delta_enhanced(stream_id, data)
                    elif event_type == "response.function_call_arguments.done":
                        await self.handle_Gemini_function_call_enhanced(stream_id, data)
                        ...
                        await self._handle_customer_interruption(stream_id, Gemini_ws)
                    elif event_type == "input_audio_buffer.speech_stopped":
                        logger.info(f"🎤 CUSTOMER STOPPED SPEAKING (enhanced) for {stream_id}")
                        # Enhanced response generation
                        await self.trigger_Gemini_response_enhanced(stream_id, Gemini_ws)
                    elif event_type == "response.done":
                        logger.info(f"✅ SARAH FINISHED RESPONSE (enhanced) for {stream_id}")
                    elif event_type == "error":
                        logger.error(f"❌ ENHANCED Gemini ERROR: {data}")
                    elif event_type == "session.updated":
                        logger.info(f"🔧 SESSION UPDATED for {stream_id}")
                        
                except json.JSONDecodeError as e:
                    logger.error(f"❌ JSON decode error from Gemini (enhanced): {e}")
                except Exception as e:
                    logger.error(f"❌ Error processing enhanced Gemini response: {e}")
                    
        except Exception as e:
            logger.error(f"❌ Error in enhanced Gemini response handler: {e}")

    async def _handle_customer_interruption(self, stream_id: str, Gemini_ws):
    ...
    async def trigger_Gemini_response_enhanced(self, stream_id: str, Gemini_ws):
        """Trigger enhanced Gemini response generation with improved parameters"""
        try:
            ...
            await Gemini_ws.send(json.dumps(response_create))
            logger.info(f"🎯 TRIGGERED ENHANCED Gemini RESPONSE for {stream_id}")
            
        except Exception as e:
            logger.error(f"❌ Error triggering enhanced Gemini response: {e}")

    async def handle_Gemini_audio_delta_enhanced(self, stream_id: str, data: dict):
        """Handle enhanced audio response from Gemini with multi-sample rate support"""
        try:
            if stream_id not in self.exotel_connections:
            ...
            sample_rate = self.connection_sample_rates.get(stream_id, self.default_sample_rate)
            output_format = self.Gemini_connections[stream_id].get("output_format", "raw/slin")
            
            # Decode audio based on format
            Gemini_audio = base64.b64decode(audio_delta)
            ...
            if output_format == "pcm16" and sample_rate >= 16000:
                ...
                exotel_pcm = Gemini_audio
            else:
                # G.711 u-law output - convert to PCM for Exotel
                exotel_pcm = self.convert_ulaw_to_pcm(Gemini_audio)
            ...
            logger.debug(f"📞 ENHANCED SARAH'S VOICE SENT: {len(Gemini_audio)} bytes {output_format} → {len(exotel_pcm)} bytes PCM @ {sample_rate}Hz")
            
        except Exception as e:
            logger.error(f"❌ Error sending enhanced audio to Exotel: {e}")

    async def handle_Gemini_function_call_enhanced(self, stream_id: str, data: dict):
        """Handle enhanced function calls from Gemini with improved error handling"""
        try:
            ...
            # Send enhanced function result back to Gemini
            Gemini_ws = self.Gemini_connections[stream_id]["websocket"]
            ...
            await Gemini_ws.send(json.dumps(function_response))
    ...
    async def cleanup_connections(self, stream_id: str):
        ...
        try:
            ...
            if stream_id in self.Gemini_connections:
                ...
                del self.Gemini_connections[stream_id]
                logger.info(f"🧹 ENHANCED Gemini CONNECTION REMOVED: {stream_id}")
...
async def main():
    """Enhanced main function to start the Gemini Realtime Sales Bot"""
    try:
        # Initialize the enhanced sales bot
        sales_bot = GeminiRealtimeSalesBot()
